...
An ad-free static inventory view for used 2022-and-newer Mercedes-Benz EQS SUV listings at Washington dealers.

**Public site:** https://timch-ms.github.io/wa-eqs-suv-finder/

The browser reads only the checked-in sanitized inventory snapshot. A guarded GitHub Actions workflow checks daily but refreshes no more than once every two Washington calendar days, uses at most 2 inventory API calls, retains the previous snapshot on failures or suspicious result counts, and then deploys an explicit static-file allowlist.
